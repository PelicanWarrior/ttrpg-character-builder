...
export default function SFCharacterCreator() {
  ...
  const loadClasses = async () => {
    ...
    try {
      ...
      const filtered = (data || []).filter((row) =>
        typeof row.DNDMod === 'string' && row.DNDMod.toLowerCase().includes('sci fi dnd')
      );
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          {showClasses && (
            <div className="w-full max-w-xl bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
              <h2 className="text-lg font-semibold text-gray-800">Classes (Sci Fi DND)</h2>
