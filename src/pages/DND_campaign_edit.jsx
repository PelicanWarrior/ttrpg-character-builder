...
export default function DNDCampaignEdit() {
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      ...
      if (!campaignId) {
        navigate(`/DND_campaign?mod=${encodeURIComponent(mod)}&ttrpgId=${encodeURIComponent(ttrpgId)}`);
        return;
      }

      const [{ data: campaign, error }, { data: chars, error: charsErr }, { data: logsData, error: logsErr }, { data: classRows }, { data: raceRows }] = await Promise.all([
        supabase
          .from('DND_campaign')
    ...
    fetchCampaign();
  }, [searchParams, navigate, mod, ttrpgId]);
  ...
  const handleNewLogBlur = async () => {
    ...
    const { data: newLog, error } = await supabase
      .from('DND_campaign_log')
      .insert([{ Log: newLogText, Log_number: nextLogNumber, campaignID: parseInt(campaignId, 10) }])
      .select()
      .single();
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-6 flex-wrap">
          ...
          <button
            onClick={() => navigate(`/DND_notes?campaignId=${searchParams.get('id')}&campaignName=${encodeURIComponent(campaignName)}&mod=${encodeURIComponent(mod)}&ttrpgId=${encodeURIComponent(ttrpgId)}`)}
            className="px-6 py-3 bg-green-600 text-white font-bold text-lg rounded-xl hover:bg-green-700 shadow-lg transition"
          >
            Campaign Notes
          </button>
          {editMode && (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {characters.map((char) => (
                <div key={char.id} className="bg-gray-800 rounded-lg p-2 text-center border border-gray-700" style={{ minWidth: '120px' }}>
                  <div
                    className="mx-auto mb-2 flex items-center justify-center rounded-full border border-red-500 text-white"
                    style={{ width: '52px', height: '52px' }}
                  >
                    <span className="text-sm font-bold">{(char.Name || 'U')[0]}</span>
                  </div>
                  ...
                  <p className="text-xs text-gray-300 leading-tight truncate" style={{ marginTop: 0 }}>
                    {(raceMap[char.Race] || 'Unknown Race')} - {(classMap[char.Class] || 'Unknown Class')}
                  </p>
                </div>
              ))}
            </div>
